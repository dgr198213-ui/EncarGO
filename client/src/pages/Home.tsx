...
const ESTADO_INFO: Record<Encargo["estado"], { label: string; pillClass: string }> = {
  ...
  fallido: { label: "Fallo en la ejecución", pillClass: "pill pill--draft" },
  ...
  async function confirmOrder() {
    ...
    try {
      ...
      if (actualizado.estado === "completado") {
        toast.success("Encargo ejecutado correctamente.");
      } else if (actualizado.estado === "fallido") {
        toast.error("El encargo se confirmó, pero la ejecución falló. Revisa el detalle.");
      } else {
  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      {view === "historial" ? <section className="content history-view">
        ...
        <div className="intro"><div className="intro-copy"><span className="eyebrow">NUEVO ENCARGO · PASO {pasoActivoLabel}</span><h1>{step === "intake" || step === "analizando" ? <>Cuéntame qué<br /><em>necesitas.</em></> : step === "mirror" ? <>Esto es lo que<br /><em>he entendido.</em></> : step === "confirmando" ? <>Ejecutando<br /><em>tu encargo.</em></> : current?.estado === "fallido" ? <>Algo no<br /><em>ha salido bien.</em></> : <>Encargo<br /><em>completado.</em></>}</h1><p>{step === "intake" ? "Escríbelo como se lo contarías a alguien de confianza. Yo me ocupo de ordenar el resto." : step === "analizando" ? "Clasificando tu petición y preparando los pasos concretos." : step === "mirror" ? "Antes de actuar, te devuelvo el encargo traducido a un plan concreto." : step === "confirmando" ? "Confirmando y ejecutando cada paso. Puede tardar unos segundos." : current?.estado === "fallido" ? "La ejecución falló en al menos un paso — puedes ver el detalle abajo." : "Cada paso se ha ejecutado y queda guardado en tu histórico."}</p></div><div className="intro-art"><div className="intro-art-placeholder" aria-hidden="true" /><div className="art-note">IDEA → ACCIÓN<br /><b>sin perder el hilo</b></div></div></div>
        <div className="workbench">
          ...
          {step === "confirmando" && <div className="progress-panel"><div className="loading-orb"><Sparkles size={25} /></div><div className="progress-copy"><span className="eyebrow">EJECUTANDO</span><h2>Un momento.</h2><p>Ejecutando cada paso contra su herramienta correspondiente.</p><div className="progress-track"><span /></div></div></div>}
          {step === "confirmado" && current && <div className="result-panel"><div className="result-top"><div><span className="step-label"><span>03</span> Resultado</span><StatusPill estado={current.estado} /></div><button className="close-result" onClick={reset}><X size={18} /></button></div><h2>{current.resumenLegible}</h2><div className="result-content"><div className="result-paper"><div className="paper-label">ENCARGO · {rutaPrincipal.toUpperCase()}</div><h3>{current.resumenLegible}</h3>{current.pasos.map((p) => <div key={p.id} style={{ marginTop: 10 }}><small style={{ opacity: .65 }}>{p.descripcion} ({p.herramientaDestino})</small><p style={{ margin: "4px 0 0", whiteSpace: "pre-wrap" }}>{(p.resultado as any)?.ok ? (p.resultado as any).contenido : (p.resultado as any)?.error ? `⚠ ${(p.resultado as any).error}` : "Pendiente de ejecución."}</p></div>)}<div className="paper-line" /><span>{current.estado === "completado" ? "Completado" : current.estado === "fallido" ? "Fallido" : "Confirmado"} · {formatearFecha(current.confirmadoEn ?? current.creadoEn)}</span></div></div><div className="result-actions"><button className="secondary-button" onClick={() => { navigator.clipboard?.writeText(current.pasos.map((p) => (p.resultado as any)?.contenido ?? "").join("\n\n")); toast.success("Resultado copiado"); }}><Copy size={16} />Copiar resultado</button><button className="primary-button" onClick={reset}><Plus size={16} />Nuevo encargo</button></div></div>}
        </div>
